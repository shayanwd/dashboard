...
export const SalesRanglist = ({ className = '', leads }) => {
        ...
        <section className={`flex w-full flex-col items-stretch justify-center px-3 sm:px-5 py-6 sm:py-8 border-[rgba(21,21,21,1)] border-b ${className}`}>
                        ...
                        <div key={index} className="bg-[rgba(255,255,255,0.09)] border p-4 rounded-[10px] border-[rgba(255,255,255,0.06)] border-solid">
                            ...
                            <div className="space-y-2 text-sm">
                                <div className="flex justify-between">
                                    <span className="text-[rgba(139,139,139,1)]">Datum:</span>
                                    <span className="text-white">{lead.datum}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-[rgba(139,139,139,1)]">Name:</span>
                                ...
                                <div className="flex justify-between">
                                    <span className="text-[rgba(139,139,139,1)]">Produkt:</span>
                                    <span className="text-white">{lead.produkt}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-[rgba(139,139,139,1)]">Setter:</span>
                                    <span className="text-white">{lead.setter}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-[rgba(139,139,139,1)]">Closer:</span>
                                    <span className="text-white">{lead.closer}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-[rgba(139,139,139,1)]">Erste rate:</span>
                                    <span className="text-white">{lead.ersteRate}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-[rgba(139,139,139,1)]">Umsatz:</span>
                                    <span className="text-white">{lead.umsatz}</span>
                                </div>
                            </div>
                        </div>
                    ))
            ...
            <div className="hidden sm:block w-full">
                ...
                <div className="flex w-full items-center gap-4 lg:gap-8 text-sm text-[rgba(139,139,139,1)] font-normal leading-none flex-wrap pl-[18px] pr-2 py-2.5 rounded-[10px]">
                    ...
                    <div className="min-w-0 flex-1 truncate">Produkt</div>
                    <div className="min-w-0 flex-1 truncate">Setter</div>
                    <div className="min-w-0 flex-1 truncate">Closer</div>
                    <div className="min-w-0 flex-1 truncate">Erste rate</div>
                    <div className="min-w-0 flex-1 truncate">Umsatz</div>
                </div>
                ...
                <div className="bg-[rgba(255,255,255,0.09)] border w-full mt-1 p-2 rounded-[10px] border-[rgba(255,255,255,0.06)] border-solid">
                    {paginatedLeads.length > 0 ? (
                        paginatedLeads.map((lead, index) => (
                            ...
                            >
                                <div className="min-w-0 flex-1 truncate">{lead.datum}</div>
                                <div className="min-w-0 flex-1 truncate">{lead.name}</div>
                                <div className="min-w-0 flex-1 truncate">{lead.email}</div>
                                <div className="min-w-0 flex-1 truncate">{lead.produkt}</div>
                                <div className="min-w-0 flex-1 truncate">{lead.setter}</div>
                                <div className="min-w-0 flex-1 truncate">{lead.closer}</div>
                                <div className="min-w-0 flex-1 truncate">{lead.ersteRate}</div>
                                <div className="min-w-0 flex-1 truncate">{lead.umsatz}</div>
                            </div>
                        ))
                    ) : (
